...
import { generateAccessToken, generateEmailVerificationToken, generateRefreshToken, } from "../utils/jwt.js";
import { generateOTP, generateResetToken, hashOTP, hashResetToken, hashToken, } from "../utils/crypto.js";
...
export const resendOTP = async (req, res) => {
    ...
    const newVerificationToken = generateEmailVerificationToken(user._id.toString());
    await user.save();
...
export const logIn = async (req, res) => {
    ...
    await user.save();
    await sendOTPEmail(user.email, "New Login Detected", `
...
export const logOut = async (req, res) => {
    const refreshToken = req.cookies.refreshToken;
    if (refreshToken) {
        await User.updateOne({ "refreshTokens.token": hashToken(refreshToken) }, { $pull: { refreshTokens: { token: hashToken(refreshToken) } } });
    }
...
export const forgetPassword = async (req, res, next) => {
    try {
        ...
        if (user.passwordReset?.lockedUntil &&
            user.passwordReset.lockedUntil > new Date()) {
            const remainingTime = Math.ceil((user.passwordReset.lockedUntil.getTime() - Date.now()) / 1000 / 60);
            return res.status(429).json({
        ...
        console.log(`✅ Password reset requested for ${user.email} from IP: ${req.ip}`);
        res.status(200).json({
            message: "If an account exists with this email, you will receive a password reset link.",
            code: "RESET_EMAIL_SENT",
        });
