...
import { logIn, signUp, refresh, getMe, logOut, forgetPassword, verifyCode, setPassword, resendOTP, } from "../controllers/auth.controller.js";
import { authenticate } from "../middlewares/authenticate.middleware.js";
import { isOwner } from "../middlewares/ownership.middleware.js";
import { authRateLimiter, signupRateLimiter, otpRateLimiter, refreshRateLimiter, } from "../middlewares/rateLimiter.middleware.js";
...
authRouter.get("/users/:id", authenticate, isOwner("id"), // check req.user.id === req.params.id OR role === admin
async (req, res) => {
...
authRouter.get("/session", authenticate, async (req, res) => {
    ...
    const sessions = user?.refreshTokens
        .filter((token) => token.expiresAt > new Date()) // only active sessions
        .map((token, index) => ({
        id: index,
...
authRouter.delete("/sessions/:id", authenticate, async (req, res) => {
    const user = await User.findById(req.user.id);
    const sessionIndex = parseInt(Array.isArray(req.params.id) ? req.params.id[0] : req.params.id);
